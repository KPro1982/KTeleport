    ...
    public class HomePoint : Portal
    {
        ...
        public HomePoint(string name, Vector3i getBlockPosition) : base(name, getBlockPosition)
        {
            ...
            if (KHelper.GetXmlProperty("KTeleport", "SaveHomeDelaySeconds") != "")
                _minDuration =
                    int.Parse(KHelper.GetXmlProperty("KTeleport", "SaveHomeDelaySeconds"));

            if (!isFirstTime) _duration = DateTime.Now.Subtract(_timeLastCreated).TotalSeconds;
            ...
            {
                KHelper.EasyLog(
                    $"Too soon. You can reset home in {(int) (_minDuration - _duration) / 60} minutes.");
                IsValid = false;
            }
        }
